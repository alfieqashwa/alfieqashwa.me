...
* `initial state value` ditulis di dalam properti `useState methods`
* `key` nya ditulis di array pertama, dalam contoh adalah `count`
* `setState` menggantikan fungsi `setState methods` dalam `class Component`
...
Mari kita konversi `Counter App` yang kita buat pada [artikel sebelumnya](/state) dengan menggunakan metodologi `Class Stateful Component` ke dalam `React Hooks Functional Stateful Component`.
