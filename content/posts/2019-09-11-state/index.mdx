...
Untuk dapat menjadikannya interaktif, kita memerlukan seorang biduan cuantik nan memesona bernama `setState`.
...
setState umumnya -- dan lebih `savety` dituliskan dalam sebuah `function`. `function` di dalam suatu `class` dinamakan `method`; kita tidak perlu menuliskan `keyword function` di dalam `class`.

Logika `method` yang kita beri nama `increment` pada `code` di atas adalah:
- Tambahkan satu angka pada apapun nilai counter.
- Tambahkan `method` di dalam `onClick` pada `button` yang mewakilkan penambahan/increment sehingga eksekusi terjadi manakala `user` mengklik tombol/button tersebut.
...
Kemudian kita buat satu `method` lagi bernama `decrement` seperti `method` `increment`, dengan `statement` matimatika berbeda.
