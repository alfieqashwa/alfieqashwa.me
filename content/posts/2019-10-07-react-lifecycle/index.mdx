...
Pada awal mempelajari `React`, saya kesulitan dalam memahami konsep `life-cycle`. Begitu banyak `methods` yang harus difahami -- dus di hapal. Penamaannya juga agak membingungkan, sebagian jarang digunakan dan menemukan pemaparan contoh-contohnya. Beberapa ternyata tidak akan dgunakan lagi pada `React version 17` yang mungkin di awal tahun akan segera di release.

Dalam [`Official Documentation`](https://reactjs.org/docs/react-component.html) dijelaskan seluruh `methods` dengan lengkap berikut [Lifecycle Diagramnya](http://projects.wojtekmaj.pl/react-lifecycle-methods-diagram/).
